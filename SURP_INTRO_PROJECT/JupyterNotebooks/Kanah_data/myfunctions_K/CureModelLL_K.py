...
from myfunctions_K import PlottingLL_K, CureModel_K
...
def CureModelLL_K(ebs, aps_0, mu, stime, Np, pv):      
...
#     plt.rc('legend', fontsize=(7))
    ...
    cure_model = CureModel_K.CureFitter0_K().fit(T, E, timeline=np.arange(1, 10000), initial_point = np.array([pv]))
...
#     cure_model.plot_cumulative_hazard(ax = ax)
    plt.title('Cure_Model (eb,ap,mu)_(KD) = {}'.format((round(ebs,3),round(aps_0,3),mu)))
    plt.savefig('Cure_Model(KMF vs. Cure_SF)(lambda_ = 100)(eb,ap,mu)_(KD)={}.png'.format((round(ebs,3),round(aps_0,3),mu)),dpi = 250)
